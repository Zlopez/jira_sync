...
def build_legacy_config(enabled):
    return {
        "General": {
            "jira_instance": "https://jira.atlassian.com",
            "jira_project": "CPE",
            "jira_token": "token",
        ...
        "Pagure": {
            ...
            "usernames": TEST_PAGURE_TO_JIRA_USERS.copy(),
        },
    }


def build_new_config(enabled):
    return {
        "general": {
            "jira": {
...
@pytest.mark.parametrize("flavor", ("legacy", "new"))
...
def test_sync_tickets(flavor, enabled, creation_fails, tmp_path, runner, caplog):
    if flavor == "legacy":
        config = build_legacy_config(enabled=enabled)
        general_config = config["General"]
        jira_config = {
            "instance_url" if key == "jira_instance" else key.replace("jira_", ""): value
            for key, value in general_config.items()
        }
        statuses_map = general_config["states"]
        pagure_usermap = config["Pagure"]["usernames"]
    else:
        config = build_new_config(enabled=enabled)
