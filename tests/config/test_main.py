...
OLD_CONFIG_PATH = PROJECT_ROOT / "config-legacy.example.toml"
...
@pytest.mark.parametrize(
    "config_file, usermap_type, config_source",
    (
        (OLD_CONFIG_PATH, "direct", "instance"),
        (CONFIG_PATH, "relative", "instance"),
        (CONFIG_PATH, "relative", "repo"),
        (CONFIG_PATH, "absolute", "instance"),
        (CONFIG_PATH, "absolute", "repo"),
        (CONFIG_PATH, "direct", "instance"),
        (CONFIG_PATH, "direct", "repo"),
    ),
    ids=lambda val: val.name if isinstance(val, Path) else val,
)
@pytest.mark.parametrize("param_type", (str, Path))
def test_main(config_file: Path, usermap_type: str, config_source: str, param_type: type, tmp_path):
