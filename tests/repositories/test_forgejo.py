...
from jira_sync.repositories import forgejo
    ...
    def test_get_next_page(self, testcase, endpoint):
        ...
        match page:
            case "first-page":
                ...
                optional_repo = (
                    "/repos/foo" if issubclass(self.cls, forgejo.ForgejoRepository) else ""
                )
        ...
        if args:
            if without_token:
                assert "AuthorizationHeaderToken" not in args["headers"]
            else:
                assert args["headers"]["AuthorizationHeaderToken"] == "token TOKEN"
...
class TestForgejoInstance(ForgejoTestBase, BaseTestInstance):
    cls = forgejo.ForgejoInstance
    ...
    def test_query_repositories(self, key, success):
        ...
        with (
            ...
            mock.patch.object(forgejo.requests, "get") as requests_get,
        ):
...
class TestForgejoRepository(ForgejoTestBase, BaseTestRepository):
    cls = forgejo.ForgejoRepository
    ...
    def test_normalize_issue(self, status, label_type):
        ...
        api_result = {
            ...
            "title": "TITLE",
            "body": "CONTENT",
            "assignee": {"login": "FORGEJO_ASSIGNEE"} if status != "new" else None,
        ...
        if status != "new":
            assert issue.assignee == "FORGEJO_ASSIGNEE"
        else:
    ...
    def test_get_issue_params(self, with_label, closed):
        ...
        if with_label:
            ...
            if closed:
                ...
                assert "state" not in params["params"]
