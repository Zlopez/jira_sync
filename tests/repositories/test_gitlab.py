...
from jira_sync.repositories import gitlab
    ...
    def test_get_next_page(self, testcase, endpoint):
        ...
        match page:
            case "first-page":
                ...
                optional_repo = (
                    "/projects/foo" if issubclass(self.cls, gitlab.GitLabRepository) else ""
                )
...
class TestGitLabInstance(GitLabTestBase, BaseTestInstance):
    cls = gitlab.GitLabInstance
    ...
    def test_query_repositories(self, key, success):
        ...
        API_RESPONSES = [
            ...
            for repo, issues_enabled, archived in chain(
        ...
        with (
            ...
            mock.patch.object(gitlab.requests, "get") as requests_get,
        ):
...
class TestGitLabRepository(GitLabTestBase, BaseTestRepository):
    cls = gitlab.GitLabRepository
    ...
    def test_normalize_issue(self, status, label_type):
        ...
        api_result = {
            ...
            "title": "TITLE",
            "description": "CONTENT",
            "assignee": {"username": "GITLAB_ASSIGNEE"} if status != "new" else None,
        ...
        if status != "new":
            assert issue.assignee == "GITLAB_ASSIGNEE"
        else:
    ...
    def test_get_issue_params(self, with_label, closed):
        ...
        if with_label:
            ...
            if closed:
                assert params["params"]["state"] == "closed"
                assert "updated_after" in params["params"]
            else:
                assert params["params"]["state"] == "opened"
                assert "updated_after" not in params["params"]
        else:
            if closed:
                assert params["params"]["state"] == "closed"
                assert "updated_after" in params["params"]
                assert "labels" not in params["params"]
            else:
                assert params["params"]["state"] == "opened"
